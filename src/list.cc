...
list_init(list_t *list) {
  list->head = reinterpret_cast<list_elm_t*>(&list->null);
  list->null = NULL;
  list->tail = reinterpret_cast<list_elm_t*>(&list->head);
}
...
list_append(list_t *list, list_elm_t *elm) {
  list->tail->next = elm;
  elm->next = reinterpret_cast<list_elm_t*>(&list->null);
  elm->prev = list->tail;
  list->tail = elm;
}
...
list_prepend(list_t *list, list_elm_t *elm) {
  list->head->prev = elm;
  elm->prev = reinterpret_cast<list_elm_t*>(&list->head);
  elm->next = list->head;
  list->head = elm;
}
