...
#define BUILDING_INDEX(ptr)  (static_cast<int>((ptr) - game.buildings))
...
#define BUILDING_PLAYER(building)  (static_cast<int>((building)->bld & 3))
...
#define BUILDING_STATE(building)  (static_cast<int>((building)->serf & 3))
/* Building is currently playing back a sound effect. */
#define BUILDING_PLAYING_SFX(building)  \
                                 (static_cast<int>(((building)->serf >> 3) & 1))
/* Building is active (specifics depend on building type). */
#define BUILDING_IS_ACTIVE(building)  \
                                 (static_cast<int>(((building)->serf >> 4) & 1))
/* Building is burning. */
#define BUILDING_IS_BURNING(building)  \
                                 (static_cast<int>(((building)->serf >> 5) & 1))
/* Building has an associated serf. */
#define BUILDING_HAS_SERF(building)  \
                                 (static_cast<int>(((building)->serf >> 6) & 1))
/* Building has succesfully requested a serf. */
#define BUILDING_SERF_REQUESTED(building)  \
                                 (static_cast<int>(((building)->serf >> 7) & 1))
/* Building has requested a serf but none was available. */
#define BUILDING_SERF_REQUEST_FAIL(building)  \
                                 (static_cast<int>(((building)->serf >> 2) & 1))

/* Building has inventory and the inventory pointer
   is valid. */
#define BUILDING_HAS_INVENTORY(building)  \
