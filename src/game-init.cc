  ...
  switch (action) {
    ...
    case ActionIncrement:
      switch (game_type) {
        ...
        case GameCustom: {
          custom_mission->set_map_size(std::min(10ul,
    ...
    case ActionDecrement:
      switch (game_type) {
        ...
        case GameCustom:
          custom_mission->set_map_size(std::max(3ul,
                                           custom_mission->get_map_size() - 1));
          break;
      }
      generate_map_preview();
...
GameInitBox::handle_click_left(int cx, int cy) {
  ...
  int lx = 0;
  int ly = 0;
  for (size_t i = 0; i < GAME_MAX_PLAYER_COUNT; i++) {
    int px = 20 + lx * 80;
    int py = 56 + ly * 80;
...
GameInitBox::handle_player_click(unsigned int player_index, int cx, int cy) {
  ...
  if ((cx < 8+32) && (cy < 72)) {
    ...
    if (!player) {
        custom_mission->add_player(std::make_shared<PlayerInfoCustom>());
    } else {
      if (player_index > 0) {
        custom_mission->remove_player(player_index);
      }
    }
  } else {
    if (!player) {
