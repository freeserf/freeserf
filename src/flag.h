...
#define FLAG_INDEX(ptr)  (static_cast<int>((ptr) - game.flags))
#define FLAG_ALLOCATED(i)  BIT_TEST(game.flag_bitmap[(i)>>3], 7-((i)&7))

/* Bitmap of all directions with outgoing paths. */
#define FLAG_PATHS(flag)  (static_cast<int>((flag)->path_con & 0x3f))
/* Whether a path exists in a given direction. */
#define FLAG_HAS_PATH(flag, dir)  \
                             (static_cast<int>((flag)->path_con & (1 << (dir))))
/* Owner of this flag. */
#define FLAG_PLAYER(flag)  (static_cast<int>(((flag)->path_con >> 6) & 3))

/* Bitmap showing whether the outgoing paths are land paths. */
#define FLAG_LAND_PATHS(flag)  (static_cast<int>((flag)->endpoint & 0x3f))
/* Whether the path in the given direction is a water path. */
#define FLAG_IS_WATER_PATH(flag, dir)  \
                            (!static_cast<int>((flag)->endpoint & (1 << (dir))))
...
#define FLAG_HAS_BUILDING(flag)  (static_cast<int>(((flag)->endpoint >> 6) & 1))
/* Whether resources exist that are not yet scheduled. */
#define FLAG_HAS_RESOURCES(flag)  \
                                 (static_cast<int>(((flag)->endpoint >> 7) & 1))

/* Bitmap showing whether the outgoing paths have transporters
   servicing them. */
#define FLAG_TRANSPORTERS(flag)  (static_cast<int>((flag)->transporter & 0x3f))
/* Whether the path in the given direction has a transporter
   serving it. */
#define FLAG_HAS_TRANSPORTER(flag, dir)  \
                          (static_cast<int>((flag)->transporter & (1 << (dir))))
/* Whether this flag has tried to request a transporter without success. */
#define FLAG_SERF_REQUEST_FAIL(flag)  \
                              (static_cast<int>(((flag)->transporter >> 7) & 1))

/* Current number of transporters on path. */
#define FLAG_TRANSPORTER_COUNT(flag, dir)  \
                                   ((unsigned int)((flag)->length[(dir)] & 0xf))
/* Length category of path determining max number of transporters. */
#define FLAG_LENGTH_CATEGORY(flag, dir)  \
                              ((unsigned int)(((flag)->length[(dir)] >> 4) & 7))
/* Whether a transporter serf was successfully requested for this path. */
#define FLAG_SERF_REQUESTED(flag, dir)  \
                            (static_cast<int>(((flag)->length[(dir)] >> 7) & 1))

/* The slot that is scheduled for pickup by the given path. */
#define FLAG_SCHEDULED_SLOT(flag, dir)  \
                              ((unsigned int)((flag)->other_end_dir[(dir)] & 7))
/* The direction from the other endpoint leading back to this flag. */
#define FLAG_OTHER_END_DIR(flag, dir)  \
                              ((dir_t)(((flag)->other_end_dir[(dir)] >> 3) & 7))
/* Whether the given direction has a resource pickup scheduled. */
#define FLAG_IS_SCHEDULED(flag, dir)  \
                     (static_cast<int>(((flag)->other_end_dir[(dir)] >> 7) & 1))

/* Whether this flag has an inventory building. */
#define FLAG_HAS_INVENTORY(flag)  \
                                (static_cast<int>(((flag)->bld_flags >> 6) & 1))
/* Whether this inventory accepts serfs. */
#define FLAG_ACCEPTS_SERFS(flag)  \
                                (static_cast<int>(((flag)->bld_flags >> 7) & 1))
/* Whether this inventory accepts resources. */
#define FLAG_ACCEPTS_RESOURCES(flag)  \
                               (static_cast<int>(((flag)->bld2_flags >> 7) & 1))
