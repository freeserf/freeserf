...
#define PLAYER_HAS_CASTLE(player)  (static_cast<int>((player)->flags & 1))
/* Whether the strongest knight should be sent to fight. */
#define PLAYER_SEND_STRONGEST(player)  \
                                  (static_cast<int>(((player)->flags >> 1) & 1))
/* Whether cycling of knights is in progress. */
#define PLAYER_CYCLING_KNIGHTS(player)  \
                                  (static_cast<int>(((player)->flags >> 2) & 1))
/* Whether a message is queued for this player. */
#define PLAYER_HAS_MESSAGE(player)  \
                                  (static_cast<int>(((player)->flags >> 3) & 1))
/* Whether the knight level of military buildings is temporarily
   reduced bacause of cycling of the knights. */
#define PLAYER_REDUCED_KNIGHT_LEVEL(player)  \
                                  (static_cast<int>(((player)->flags >> 4) & 1))
/* Whether the cycling of knights is in the second phase. */
#define PLAYER_CYCLING_SECOND(player)  \
                                  (static_cast<int>(((player)->flags >> 5) & 1))
...
#define PLAYER_IS_AI(player)  (static_cast<int>(((player)->flags >> 7) & 1))


/* Whether player is prohibited from building military
   buildings at current position. */
#define PLAYER_ALLOW_MILITARY(player)  (!static_cast<int>((player)->build & 1))
/* Whether player is prohibited from building flag at
   current position. */
#define PLAYER_ALLOW_FLAG(player)  \
                                 (!static_cast<int>(((player)->build >> 1) & 1))
/* Whether player can spawn new serfs. */
#define PLAYER_CAN_SPAWN(player)  (static_cast<int>(((player)->build >> 2) & 1))
