...
static midi_t current_track = (midi_t) - 1;
...
sfx_play_clip(sfx_t sfx)
{
	...
	if (NULL == audio_clip) {
		...
		size_t size = 0;
		char *data = (char *) gfx_get_data_object(DATA_SFX_BASE + sfx, &size);
...
midi_grow(midi_file_t *midi, uint8_t **current)
{
	...
	if (midi->data) {
		memcpy(data, midi->data, (uint)midi->size);
		free(midi->data);
	}
	*current = data + pos;
	midi->data = data;
...
midi_play_track(midi_t midi)
{
	...
	if (NULL == track) {
		...
		size_t size = 0;
		char *data = (char *) gfx_get_data_object(DATA_MUSIC_GAME + midi, &size);
		if (NULL == data) {
			free(track);
			return;
		}
