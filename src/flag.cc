...
flag_proxy_alloc(flag_t *flag) {
  flag_proxy_t *proxy =
                  reinterpret_cast<flag_proxy_t*>(malloc(sizeof(flag_proxy_t)));
  if (proxy == NULL) abort();
...
flag_search_add_source(flag_search_t *search, flag_t *flag) {
  ...
  list_append(&search->queue, reinterpret_cast<list_elm_t*>(flag_proxy));
  flag->search_num = search->id;
}

int
flag_search_execute(flag_search_t *search, flag_search_func *callback, int land,
                    int transporter, void *data) {
  for (int i = 0; i < SEARCH_MAX_DEPTH && !list_is_empty(&search->queue); i++) {
    flag_proxy_t *proxy =
              reinterpret_cast<flag_proxy_t*>(list_remove_head(&search->queue));
    ...
    for (int i = 0; i < 6; i++) {
      if ((!land || !FLAG_IS_WATER_PATH(flag, 5-i)) &&
        ...
        flag_proxy_t *other_flag_proxy =
                                  flag_proxy_alloc(flag->other_endpoint.f[5-i]);
        list_append(&search->queue,
                    reinterpret_cast<list_elm_t*>(other_flag_proxy));
      }
    }
  }
...
int
flag_search_single(flag_t *src, flag_search_func *callback, int land,
                   int transporter, void *data) {
